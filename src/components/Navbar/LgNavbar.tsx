...
export default function LgNavbar({ categories = [] }: { categories : SanityDocument[] }) {
      ...
      <div className=" flex justify-between mx-auto container">
        ...
        <div>
          <ul className="flex gap-6 p-7 my-auto uppercase text-sm ">
            ...
            <li className="hover:text-primary-3 cursor-pointer flex"
              ...
              {showDropdown && (
                <ul className="absolute bg-white text-black mt-12 border border-t-primary-3 p-2 shadow">
                  {categories.map((category)=>(
                    <Link key={category._id} href={`/blog/category/${(category.title).toLowerCase()}`}><li className="hover:text-primary-3 cursor-pointer p-2">{category.title}</li></Link>
                  ))}
                </ul>
              )}
            </li>
